...
province_diz = {}
province = set()
infile = open('regpie-studenti.csv','r')
...
for line in infile:
    ...
    if line[1] in province_diz: province_diz[line[1]] += int(line[-1])
    else : province_diz[line[1]] = int(line[-1])

    if line[3] == "1 - Scuola dell'infanzia" : infanzia += int(line[-1])

print('Le province per le quali vengono fornite le statistiche sono:')
for città in sorted(province): print(città)
print()
for città in sorted(province): print(f'Totale studenti iscritti {città}: {province_diz[città]}')
print(f"Gli studenti iscritti alla scuola dell'infanzia in Piemonte sono: {infanzia}")