...
def main():
    ...
    professioni = {}
    professioni_lista = []
    infile = open('instagram.csv','r')
    for line in infile:
        line = line.strip().split(';')
        if line[-1] not in professioni:
            professioni[line[-1]] = float(line[-2])
        else: professioni[line[-1]] += float(line[-2])
    infile.close()

    for chiavi in professioni:
        professioni_lista.append((chiavi,professioni[chiavi]))
    professioni_lista.sort(key=itemgetter(1),reverse=True)

    for elementi in professioni_lista:
        print(f'{f'{elementi[0]}:':35}{round(elementi[1],2)}M')
