...
def main():
    ...
    n = 3
    codici = {}
    infile = open('stockOld.csv','r')
    for line in infile:
        line = line.strip().split(';')
        if (line[1],line[2]) not in codici:
            codici[(line[1],line[2])] = [line[0]]
        else: codici[(line[1],line[2])].append(line[0])
    infile.close()
    print()

    infile = open('stockTrasferimento.csv','a')
    for valori in codici:
        if len(codici[valori]) > n:
            infile.write(f'{valori[0]};{valori[1]}')
            for volte in range(len(codici[valori]) - n): 
                infile.write(f';{codici[valori][volte]}')
                codici[valori].pop(volte)
            infile.write('\n')
        else: print(f'non ci sono abbastanza {valori[0]} ({valori[1]}) per essere trasferiti')
    infile.close()

    codici_lista = sorted(codici,key=itemgetter(0))
    infile = open('stockNew.csv','a')
    for valori in codici_lista:
        infile.write(f'{valori[0]};{valori[1]}')
        for volte in range(len(codici[valori])): 
            infile.write(f';{codici[valori][volte]}')
        infile.write('\n')
    infile.close()
