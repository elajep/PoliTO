...
for line in infile:
    ...
    giocatori = line[:2]
    punti = line[2:]
    for i in range(len(punti)): punti[i] = int(punti[i])

    for i in range(len(giocatori)):
        if giocatori[i] not in squadre:
            squadre[giocatori[i]] = [punti[i],punti[i-1]]
            if punti[i] == punti[i-1]: squadre[giocatori[i]].append(1)
            if punti[i] > punti[i-1]: squadre[giocatori[i]].append(3)
            if punti[i] < punti[i-1]: squadre[giocatori[i]].append(0)

        else:
            squadre[giocatori[i]][0] += punti[i]
            squadre[giocatori[i]][1] += punti[i-1]
            if punti[i] == punti[i-1]: squadre[giocatori[i]][2] += 1
            if punti[i] > punti[i-1]: squadre[giocatori[i]][2] += 3
            if punti[i] < punti[i-1]: squadre[giocatori[i]][2] += 0
infile.close()

lista_squadre = []
for squadra in squadre:
    lista_squadre.append([squadra,squadre[squadra][0],squadre[squadra][1],squadre[squadra][2]])


lista_squadre.sort(key=itemgetter(3),reverse=True)

n = 1
for squadra in lista_squadre:
    print(f'{n}. {squadra[0]} - Punti:{squadra[3]}, Gol fatti: {squadra[1]}, Gol subiti: {squadra[2]} ')
    n += 1
